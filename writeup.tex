...
\subsection{Choosing $\vec{u}$ and $\vec{v}$ for the ellipse} Another
lingering question: our choice of $\vec{u}$ and $\vec{v}$ seem to be
affecting our reduced dimension basis and therefore our projected
ellipse.  We did the same exact pca process several times with a
different 4-D plane each time and got wildly different results.
...
\label{sec:svd} So, we know that the process of singular value
...
\end{equation} where A is the matrix you want to decompose.  For our
...
\label{sec:sigma} The second output matrix of the numpy SVD function,
$\Sigma$ is a pseudo-diagonal matrix that contains the singular values
of the data matrix A, or the square roots of the eigenvalues of
$A^{T}A$.
...
\label{sec:results2} So, after talking to Gabe we found out that the
